...
public class MyHealthCheck implements HealthCheck {
    ...
    /**
     * Verificação de saúde agendada para executar a cada 30 segundos.
     * 
     ...
     */
    @Scheduled(every = "30s") // Agenda a verificação de saúde para executar a cada 30 segundos
    public void scheduledHealthCheck() {
        ...
        if (isUp) {
            ...
            ServiceState.setActiveMQActive(false);
            checkAndEstablishConnection(); // Tenta reconectar se o serviço estiver "down"
    ...
    /**
     * Tenta estabelecer uma conexão com o ActiveMQ.
     *
     * Este método tenta se conectar ao broker ActiveMQ usando as configurações fornecidas.
     * Se a conexão for bem-sucedida, o método inicializa a sessão e a conexão.
    ...
    private boolean checkServiceHealth() {
        ...
        try {
            ...
            // Se houver um erro, define a sessão como null para indicar que não está mais ativa
            closeConnection();
            return false;
        }
    }
    ...
    /**
     * Fecha a conexão e a sessão se existirem, para evitar vazamentos de recursos.
     */
    ...
    private void handleServiceDown() {
        ...
        if (!isActiveMQUp) {
            LOGGER.error("ActiveMQ ainda está 'down' após " + MAX_RETRIES + " tentativas.");
        } else {
            LOGGER.info("ActiveMQ voltou a ficar ativo.");
            ServiceState.setActiveMQActive(true);
        }
